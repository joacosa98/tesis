...
Robotito es una plataforma educativa de software libre y hardware abierto
utilizada para introducir conceptos de robótica y pensamiento computacional
en niños de nivel inicial y primeros años de primaria. En su versión
original, el robot cuenta con sensores básicos —de color y de distancia—
que limitan el tipo de actividades que se le pueden proponer a los
estudiantes.

Este trabajo presenta el diseño e implementación de una extensión de
Robotito que incorpora capacidades de procesamiento de imágenes mediante
una cámara HuskyLens, ampliando así su capacidad de percibir el entorno.
La solución integra tres componentes principales: un módulo de
comunicación I2C que conecta la cámara a la placa ESP32 del robot y
permite el seguimiento de líneas y la lectura de marcadores visuales
(AprilTags); un sistema de control del comportamiento basado en Árboles
de Comportamiento (\textit{Behavior Trees}), implementado como una
biblioteca reutilizable en Lua; y un configurador web que permite a
los docentes definir secuencias de actividad y transferirlas al robot
de forma inalámbrica mediante comunicación Bluetooth Low Energy (BLE),
con persistencia de datos en la memoria no volátil del ESP32.

A partir de estas capacidades se diseñó una actividad educativa en la
que Robotito recorre un circuito siguiendo una línea, se detiene en
distintas estaciones marcadas con tags y valida si el orden de las
tarjetas ubicadas por los estudiantes corresponde a una secuencia
previamente configurada por el docente, brindando retroalimentación
visual inmediata mediante luces LED.

La propuesta fue validada mediante entrevistas con docentes de educación
inicial y primaria, y a través de dos sesiones prácticas con estudiantes
de distintas edades en contextos reales de aula. Los resultados obtenidos
muestran que la solución es técnicamente viable, resulta comprensible y
motivadora para los niños, y permite integrar contenidos curriculares
diversos. A partir del \textit{feedback} recibido se realizaron ajustes
al diseño original, entre ellos la incorporación de funciones de pausa y
reinicio que otorgan a los docentes mayor control sobre el flujo de la
actividad en el aula.

Este trabajo constituye un aporte concreto a la plataforma Robotito,
sentando las bases para futuras extensiones que aprovechen la percepción
visual y la arquitectura modular desarrollada.

\vspace{1cm}
\noindent\textbf{Palabras clave:} Robótica educativa,  procesamiento de
imágenes, Robotito, ESP32, Husky-Lens, I2C, Lua RTOS, C++, Árboles de comportamiento, Bluetooth Low Energy, NVS
...
Para resolver esto se sumaron tres piezas a Robotito: una cámara HuskyLens (módulo de procesamiento de imágenes, capaz de seguir líneas y reconocer tags sin sobrecargar al microcontrolador del robot), una aplicación web donde el docente configura la actividad, y un canal BLE (\textit{Bluetooth Low Energy}, una variante de Bluetooth pensada para enviar poca cantidad de datos consumiendo muy poca energía) que transporta esa configuración hacia el robot. Con las tres piezas funcionando juntas, se logra plantear una actividad donde Robotito puede realizar un recorrido siguiendo una línea contínua, reconoce estaciones marcadas con tags (de tipo \textit{AprilTag}) y valida si una secuencia armada por los estudiantes es correcta. El motivo de cada una de estas elecciones técnicas se explica con más detalle en los capítulos siguientes.

\section{Actividad propuesta}

La actividad desarrollada consiste en un recorrido guiado por una línea donde Robotito detecta tags en estaciones y valida el orden de una secuencia previamente configurada por el docente. El docente prepara la actividad desde el configurador web y transfiere la configuración al robot utilizando BLE; El objetivo es que los estudiantes ubiquen tarjetas que contienen tags en las estaciones definidas en el tablero y observen la retroalimentación del robot (acierto o error) en base a la posición de cada una en el recorrido. Las especificaciones de la ejecución del robot se detallan con mayor profundidad en el Capítulo 3.

\section{Alcance funcional y límites}
...
\begin{itemize}
    \item HuskyLens: seguimiento de línea y lectura de tags (percepción acotada).
    \item Configuración: definida externamente y transferida vía BLE.
    \item Control: lógica organizada mediante Árboles de Comportamiento (\textit{Behavior Trees}), como capa de orquestación.
\end{itemize}
...
Resolverlo implica varias cosas a la vez. Del lado técnico, hay que conectar una cámara (HuskyLens) a la placa ESP32 Thing de Robotito mediante el protocolo I2C y hacerla funcionar dentro de un entorno Lua RTOS, donde se ejecuta el firmware del robot, asegurando el correcto funcionamiento del codigo ya existente. Del lado del diseño, hace falta crear una capa de control que coordine lo que la cámara detecta con lo que el robot ejecuta, es ahí donde entran los árboles de comportamiento. No menos importante, hay que diseñar una actividad educativa concreta que le dé sentido a toda esa integración: no alcanza con que el robot "vea", tiene que haber una dinámica de aula donde esa percepción se use.

\section{Objetivos}

El objetivo general es desarrollar una extensión de la plataforma Robotito que permita ejecutar una actividad educativa utilizando las capacidades de percepción visual brindada por la cámara HuskyLens, como seguimiento de línea y lectura de tags, además del procesamiento de los datos obtenidos por parte del robot, configuración inalámbrica desde una aplicación web y un sistema de control basado en árboles de comportamiento (\textit{Behavior Trees}).
...
\begin{itemize}
    ...
    \item Desarrollar una actividad educativa basada en las capacidades de lectura de HuskyLens: seguimiento de línea, lectura de tags y validación de secuencias por parte del procesamiento del tobot.
...
Para comprobar que todo esto funciona más allá de la teoría, se llevaron adelante dos instancias con maestras y niños, en las que se puso a prueba el robot en un escenario similar al de una clase real. Esas instancias se describen con más detalle en \ref{cap:experimentacion}
...
Finalmente, el apéndice reúne material complementario de referencia
técnica, incluyendo detalles de implementación de los distintos módulos,
documentación de apoyo para el usuario, y contenido visual adicional del
proyecto.
...
El equipo de Robotito documentó una serie de actividades organizadas de forma incremental en un libro destinado a docentes \cite{robotito2019libro}.
...
Existen en el mercado varios robots educativos para niños de nivel inicial que sirvieron de referencia para definir el alcance de este proyecto. 

Bee-Bot \cite{beebot} se controla mediante botones físicos sobre el propio robot que permiten almacenar una secuencia de movimientos: avanzar, retroceder, girar. Cubetto \cite{cubetto}, en cambio, utiliza un tablero externo con bloques de colores insertables que codifican cada instrucción antes de ejecutar el recorrido, de forma similar a como Robotito se programa disponiendo tarjetas en su entorno.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/antecedentes/bee-bot.jpg}
        \caption{Bee-Bot}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/antecedentes/cubetto.jpg}
        \caption{Cubetto}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/antecedentes/qobo.png}
        \caption{Qobo}
    \end{subfigure}
    \caption{Robots educativos en el mercado.}
...
Para cumplir con los requisitos planteados se propuso una arquitectura organizada en tres dimensiones principales: integración física, percepción visual y control del comportamiento. Esta separación permite abordar la extensión de Robotito como un rediseño parcial de su arquitectura, sin reemplazar la plataforma existente ni modificar innecesariamente sus componentes centrales.

En primer lugar, la libertad de elección sobre qué herramienta usar permitió encontrar una alternativa (cámara HuskyLens) con dimensiones correctas y entrenamiento previo capaz de reconocer diversos elementos del entorno físico: líneas, tags, colores y algunos objetos predeterminados. Además, la ubicación de la cámara con respecto a Robotito permite reconocer elementos que no se encuentran directamente debajo del mismo, sino que hay cierto rango de visión sobre el que se pudo trabajar que dio lugar a actividades con elementos que rodean al robot. Adicionalmente, la cámara HuskyLens cuenta con dos luces LED ubicadas en su parte frontal que de cierta manera previenen algunos errores generados por iluminación no ideal, aúnque luego de pruebas realizadas se observa que puede haber inconsistencias. La incorporación de la cámara también requirió considerar la adaptación física de Robotito. La ubicación del sensor debía permitir una lectura adecuada del entorno pero de manera que el montaje permaneciera estable, seguro y compatible con el uso del robot en contextos educativos.

Por otro lado, la solución adoptada se basa en una arquitectura de procesamiento distribuido a nivel funcional, donde la percepción visual se delega a un módulo especializado y el ESP32 conserva el rol de coordinación y control. De esta forma, el microcontrolador no procesa imágenes crudas, sino que interpreta resultados visuales de alto nivel y los utiliza dentro de la lógica de control del robot.

\subsection{Módulos a implementar}

Se define por lo tanto que la solución debe contener la lógica organizada en módulos específicos para facilitar su desarrollo y futura extensión, así como separar correctamente las responsabilidades de cada componente. La implementación de cada uno se describe en detalle en el Capítulo 4.
...
\subsection{Actividad propuesta}
...
% TODO: agregar imágenes de ejemplo de tablero y tarjetas

Definidos los tres elementos principales, la actividad a desarrollar consiste en una serie de pasos. En primer lugar, el docente debe configurar previamente la secuencia que será verificada por Robotito en el recorrido. Esto se traduce a definir una lista ordenada de tags a ser leída por Robotito durante el recorrido (un tag de cada tarjeta a ubicar en el tablero) y enviarla al robot. Este paso es explicado con más detalle en la sección correspondiente al configurador web del Capítulo 4. En resúmen, en esta parte el docente define qué tarjeta debe ir ubicada en qué lugar del recorrido.
...
A partir de estos criterios, se identificaron varias alternativas disponibles en el mercado, las cuales se describen en \ref{sec:tabla_comparativa_camaras}. Luego de evaluar cada opción, se seleccionó la cámara HuskyLens, ya que cumple con todos los requisitos establecidos. 
...
Una ventaja de esta elección es que no fue necesario desarrollar una nueva implementación del protocolo I2C, sino que se reutilizó la infraestructura de software ya existente en el proyecto. En \ref{tab:huskylens_pines_i2c} y \ref{tab:conexion_huskylens_esp32}  se presenta la asignación de los pines utilizados para establecer la conexión entre la cámara y la placa ESP32.
...
La implementación se basa en la biblioteca oficial de Arduino para HuskyLens \cite{dfrobot2021huskylens}, adaptada al entorno ESP32/FreeRTOS y a la capa de comunicación I2C del sistema. En esta sección se describe el funcionamiento del módulo y su implementación a alto nivel. Para una descripción más detallada, puede consultarse el repositorio de \cite{robotito2026hardware}, donde se encuentra el código completo.
...
La \textbf{Capa 1}, correspondiente al protocolo de comunicación, implementa el protocolo utilizado por la cámara. Su función es construir los mensajes que serán enviados a HuskyLens e interpretar las respuestas recibidas, verificando que la información obtenida sea válida antes de entregarla al resto del sistema. Esta capa no realiza acceso directo al hardware, sino que trabaja únicamente con el formato de los datos intercambiados.

Sobre esta se encuentra la \textbf{Capa 2}, correspondiente a la interfaz de acceso en C, que constituye el núcleo del módulo. Es la responsable de inicializar la comunicación con la cámara, enviar las solicitudes correspondientes, recibir las respuestas e interpretar la información obtenida. Además, mantiene internamente los resultados de las detecciones para que puedan ser consultados posteriormente por la aplicación.

Finalmente, la \textbf{Capa 3}, correspondiente a la interfaz para Lua, proporciona la funcionalidad utilizada por los programas escritos en este lenguaje. Su objetivo es adaptar las funciones implementadas en C al entorno de ejecución de Lua empleado por Robotito, ofreciendo una interfaz sencilla y ocultando los detalles internos de la comunicación con la cámara.

El funcionamiento general del módulo comienza cuando la aplicación solicita información a la cámara mediante la interfaz disponible en Lua o directamente desde código C. La Capa 2 prepara la solicitud y la envía utilizando el controlador I2C del sistema. Una vez recibida la respuesta de HuskyLens, el módulo la procesa con ayuda de la Capa 1, interpreta la información obtenida y actualiza su estado interno. Posteriormente, la aplicación puede consultar estos resultados sin necesidad de interactuar directamente con el protocolo de comunicación.

Esta organización permite que cada capa se ocupe de una tarea específica: la Capa 1 implementa el protocolo de comunicación, la Capa 2 coordina el intercambio de información con la cámara y la Capa 3 expone una interfaz para la aplicación. Como resultado, se obtiene una implementación modular, sencilla de mantener y consistente con la arquitectura del resto del software de Robotito.
...
Para conocer en profunida la implementación del modulo de BT, puede visitar el repositorio con su implementación completo \cite{robotito2026software}
...
Reforzar el alcance real del proyecto.
Describir el aporte como una extensión concreta de Robotito:
percepción visual acotada a línea y tags;
configuración inalámbrica de actividades;
control modular mediante Behavior Trees;
integración en una actividad educativa.
Presentar el trabajo futuro como extensión natural del alcance actual.
...
\begin{table}[ht]
    \centering
    \small
    \renewcommand{\arraystretch}{1.35} % más aire entre filas
