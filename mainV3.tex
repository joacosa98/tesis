...
En el Capítulo 8 se presenta la creación de la actividad: reglas, dinámica y objetivos pedagógicos. Se detalla el diseño del circuito sobre tatami, el seguimiento de línea, la detección de marcas rojas, la lectura de códigos QR y la verificación de secuencias, los modos pausa y reset, la implementación de la actividad mediante BT y los objetivos pedagógicos de la actividad. \\
...
\chapter{Creación de la actividad (reglas, dinámica y objetivos pedagógicos)}
...
\section{Diseño y dinámica de la actividad}
...
\begin{figure}[!htbp]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1\textwidth]{images/tablero/robotito-tablero-recorrido.png}
    }
    \caption{Tablero de actividad}
    \label{fig:tablero}
\end{figure}

\FloatBarrier

\subsection{Reglas de la actividad}

Las reglas de la actividad se describen a continuación:

\begin{itemize}
    \item \textbf{Configuración}: el robot se enciende y activa el modo configuración. En este estado puede recibir datos de configuración por bluetooth. Permanece en este estado hasta recibir la señal de configuración finalizada.

    \item \textbf{Inicio}: el robot se coloca en el inicio. Permanece en estado de espera activo hasta recibir la señal de comienzo. Una vez detectada esta señal, inicia el recorrido.
    ...
    \item \textbf{Reseteo y estado de espera}: una vez finalizada la verificación global, el robot vuelve a colocarse en la casilla de inicio donde lee la señal de reset y entra en modo de espera. Aquí se puede iniciar un nuevo recorrido.
\end{itemize}

\subsection{Modo pausa y modo reset}

Además del comportamiento principal del recorrido, el robot incorpora dos modos especiales de operación que permiten al docente intervenir en cualquier momento de la actividad sin perder el hilo de la actividad: \emph{pausa} y \emph{reset}.
...
\begin{itemize}
    ...
    \item \textbf{Reanudación de la actividad}: al volver a apoyar el robot en el piso, éste queda a la espera de una señal externa. Si recibe la señal de \textbf{continuar}, retoma el recorrido exactamente en la etapa en la que lo había dejado antes de ser levantado, sin perder los datos de las estaciones ya evaluadas.
...
\begin{itemize}
    ...
    \item \textbf{Continuidad pedagógica}: el modo pausa permite explicar conceptos en el momento exacto en que surgen, sin perder el contexto de la actividad.
    
    \item \textbf{Gestión eficiente del tiempo de clase}: el modo continuar evita repetir partes de la actividad innecesariamente, evitando posible frustraciones o aburrimiento por parte de los estudiantes.
    
    \item \textbf{Flexibilidad en la dinámica de la actividad}: el modo reset habilita comenzar una nueva partida de forma instantánea, facilitando la repetición de actividades, la comparación de resultados o la experimentación con variantes del recorrido.
...
\subsection{Implementación de la actividad utilizando BT}
...
La lógica de control de la actividad fue implementada utilizando Árboles de Comportamiento (Behavior Trees, BT), debido a las ventajas que esta arquitectura ofrece en términos de modularidad, reactividad y claridad estructural. A diferencia de una Máquina de Estados Finitos (FSM), donde las transiciones entre estados pueden crecer exponencialmente a medida que aumenta la complejidad del sistema, los BT permiten encapsular comportamientos independientes en nodos reutilizables y organizarlos jerárquicamente.
...
La interrupción monitorea permanentemente condiciones críticas del robot durante toda la ejecución de la actividad. Entre los eventos supervisados se encuentran:
...
Su responsabilidad es gestionar el ciclo de vida completo del robot dentro de la actividad, organizando las diferentes etapas de funcionamiento:
...
Durante esta etapa el robot permanece en modo Standby hasta detectar el tag o condición visual que habilita el comienzo de la actividad. Esto permite que el docente o los estudiantes preparen el circuito antes de iniciar la ejecución.
...
La dinámica de la actividad involucra activamente a los estudiantes:
...
En conjunto, el diseño de la actividad sirve como puente entre el razonamiento lógico, la robótica educativa y la interacción con un sistema autónomo de manera accesible y motivadora.
...
\textbf{Persistencia de modelos aprendidos en tarjeta SD.} Los modelos entrenados en la HuskyLens ---línea guía, etiquetas de control y etiquetas de parada--- se almacenan en la memoria interna del dispositivo y pueden perderse si un usuario manipula accidentalmente el menú de la cámara. Guardarlos en una tarjeta SD permitiría restaurarlos automáticamente al encender el robot, eliminando la necesidad de reentrenar el sensor ante cada desconfiguración.
