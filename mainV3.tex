...
\hfill \break
\keywords{Robótica educativa, Pensamiento computacional, ESP32, HuskyLens, I2C, Lua RTOS, C++, Árboles de comportamiento}
...
La experiencia internacional y regional demuestra que la robótica fomenta la curiosidad, el trabajo colaborativo y la experimentación guiada. Sin embargo, su verdadero valor no reside únicamente en la capacitación técnica, sino en la posibilidad de aprender a pensar con tecnología, comprendiendo sus implicaciones cognitivas y sociales.
...
Desarrollar una extensión de la plataforma educativa Robotito que integre una cámara inteligente HuskyLens y un sistema de control basado en árboles de comportamiento (\textit{Behavior Trees}), implementando una librería modular y reutilizable que sirva como base para futuros desarrollos robóticos y educativos.
...
\begin{itemize}
    ...
    \item Diseñar e implementar un módulo propio de \textit{Behavior Trees}, que funcione como librería de control general reutilizable para futuros proyectos.
    ...
    \item Validar el rendimiento técnico de la librería de \textit{Behavior Trees} y la integración del sensor en el ESP32.
\end{itemize}
...
Desde el punto de vista técnico, desarrolla una librería de \textit{Behavior Trees} adaptada al entorno de Robotito, demostrando su eficacia como herramienta flexible para modelar comportamientos robóticos. Además, integra una cámara HuskyLens con una placa ESP32, creando un módulo que permite interactuar con la misma.

Desde el punto de vista pedagógico, promueve la enseñanza de estructuras jerárquicas de control y razonamiento lógico, acercando a los estudiantes a prácticas propias de la robótica contemporánea.

En conjunto, el proyecto consolida a Robotito como una plataforma que integra visión artificial, control modular y aprendizaje activo, alineada con las tendencias internacionales en educación STEM.
...
En el Capítulo 4 se describe el módulo HuskyLens: su hardware, conexión con la placa ESP32 por I2C, el protocolo de aplicación, la arquitectura del software (núcleo de protocolo, capa en C y binding Lua), el formato de datos que proporciona, los algoritmos de visión soportados y su uso desde la aplicación en C y Lua. \\

En el Capítulo 5 se exponen los fundamentos teóricos de los árboles de comportamiento (\textit{Behavior Trees}) y se documenta la implementación del motor BT como librería reutilizable, incluyendo los tipos de nodos (acción, condición, compositores), la reactividad y la integración con el sistema. \\

En el Capítulo 6 se presenta la creación del juego: reglas, dinámica y objetivos pedagógicos. Se detalla el diseño del circuito sobre tatami, el seguimiento de línea, la detección de marcas rojas, la lectura de códigos QR y la verificación de secuencias, los modos pausa y reset, la implementación del juego mediante BT y los objetivos pedagógicos de la actividad. \\

En el Capítulo 7 se documenta el modelado de la carcasa para Robotito, desarrollado para integrar de forma robusta y reproducible la cámara HuskyLens, cumpliendo requisitos de posición, ángulo y guiado del cableado. \\

En el Capítulo 8 se documenta la experimentación realizada, tanto desde el punto de vista técnico como pedagógico: entrevistas con docentes, sesión práctica con estudiantes, análisis de resultados, ajustes realizados a partir del feedback y conclusiones del proceso de experimentación. \\

En el Capítulo 9 se presentan las conclusiones generales y las líneas de trabajo futuro, destacando los aportes del proyecto tanto en el desarrollo de una herramienta tecnológica accesible como en la generación de propuestas pedagógicas innovadoras que fortalezcan el aprendizaje de la robótica en edades tempranas. \\

Al final del documento se incluye un Anexo con material complementario.
...
\section{Marco teórico y fundamentos del aprendizaje activo}

\subsection{Fundamentos psicopedagógicos clásicos: objeto tangible y andamiaje visual}
...
\section{Marcos de integración disciplinar y cognitiva}

\subsection{Articulación curricular STEM/STEAM}

La robótica educativa representa una de las plataformas de instrumentación más eficientes para los marcos integrados de Ciencia, Tecnología, Ingeniería y Matemática (STEM) \cite{ncsesNDstem}. Un robot móvil unifica de manera nativa conceptos transdisciplinares: variables físicas y mediciones sensoriales (Ciencia), entornos de desarrollo y lógica computacional (Tecnología), diseño mecatrónico y optimización de sistemas bajo restricciones (Ingeniería), y nociones espaciales, proporcionalidad y geometría de trayectorias (Matemática). La transición contemporánea hacia el enfoque STEAM incorpora la dimensión del diseño, las artes expresivas y las capas de modelado conceptual \cite{yakman2019steam}.  Desde una perspectiva de ingeniería de interfaces, la capa \enquote{Arts/Design} no constituye un aditivo estético secundario, sino que define la envoltura de usabilidad del dispositivo: justifica el diseño de carcasas modulares impresas en 3D y la creación de narrativas interactivas que alteran significativamente los patrones de interacción y la ergonomía didáctica del sistema, sin complejizar la arquitectura de hardware base.

\subsection{Taxonomía del pensamiento computacional aplicada}
...
\subsection{Arquitecturas de control: Máquinas de Estados Finitos vs. Árboles de Comportamiento}
...
La revisión sistemática de los antecedentes teóricos, históricos y tecnológicos permite concluir de manera concluyente que la robótica educativa constituye un dominio maduro y multidisciplinar estructurado sobre sólidos pilares de aprendizaje constructivista, construccionista y socio-cultural. La efectividad de una plataforma no reside en la complejidad aislada de su hardware, sino en la coherencia de su ecología de mediación: la articulación armónica entre una interfaz de software intuitiva (andamiaje), una arquitectura mecatrónica robusta y de bajo costo (accesibilidad), y un marco metodológico estructurado que traduzca la manipulación física en operaciones explícitas de pensamiento computacional.

Las trayectorias de éxito analizadas (como el Plan Ceibal y el proyecto Butiá en el entorno nacional) demuestran que el desarrollo local apoyado en principios de hardware libre y software de código abierto es una condición indispensable para asegurar la soberanía tecnológica y la inclusión socioeducativa sostenible en la región. Las tendencias contemporáneas que introducen la visión artificial y las arquitecturas de control adaptativo (Árboles de Comportamiento) incrementan exponencialmente el potencial del robot como objeto para pensar, pero a su vez imponen demandas críticas de diseño sobre las interfaces de usabilidad y la retroalimentación en tiempo real para evitar que el dispositivo actúe como una caja negra opaca. Esta rigurosa delimitación de los antecedentes científicos y mecatrónicos fundamenta de manera unívoca las decisiones experimentales, de diseño de firmware, optimización de hardware y validación empírica en territorio que se detallarán y justificarán en los capítulos subsecuentes de la presente investigación.
...
\section{Contexto y criterios de selección}

Dado que uno de los requisitos fundamentales del proyecto es la utilización de procesamiento de imágenes, resulta esencial seleccionar un módulo de cámara adecuado para la interpretación visual del entorno.
...
Los aspectos específicos del diseño de la carcasa, el soporte y la validación física se desarrollan en el capítulo correspondiente al modelado de la carcasa para Robotito.
...
\hline
Evitar acoplamiento entre hardware y lógica del juego &
...
El proyecto incorpora una cámara de visión artificial \textbf{HuskyLens} (DFRobot) para dotar al sistema de capacidades de detección visual: reconocimiento de rostros, seguimiento de objetos, detección de líneas, reconocimiento de colores o de marcas (tags), entre otros. Para integrar este dispositivo con el ESP32 que ejecuta Lua-RTOS se desarrolló un \textbf{módulo de software en C} que encapsula la comunicación con el HuskyLens y expone una API usable tanto desde código C como desde scripts Lua. \\

La implementación se basó en la librería oficial de Arduino para HuskyLens \cite{dfrobot2021huskylens}, adaptada al entorno ESP32/FreeRTOS y a la capa I2C del sistema. En esta sección se describe \emph{cómo funciona} el módulo y \emph{cómo está implementado}. \\
...
Muchos dispositivos I2C utilizan un modelo de ``registros'': el maestro envía la dirección de un registro y luego lee o escribe datos. El HuskyLens utiliza un \textbf{protocolo orientado a mensajes}: cada intercambio es una \textbf{trama completa} (frame) con cabecera, comando, contenido y checksum. No hay byte de ``registro''; el primer byte útil es ya parte del protocolo (cabecera \texttt{0x55 0xAA}). Por ello la implementación usa \textbf{transferencias I2C sin byte de registro}: dirección del esclavo seguida del payload en escritura, y en lectura se solicitan N bytes y se lee la respuesta. Este esquema coincide con el de la librería Arduino (\texttt{Wire}) \cite{dfrobot2021huskylens}. En el código se usan las primitivas del driver I2C del sistema (\texttt{i2c\_start}, \texttt{i2c\_write\_address}, \texttt{i2c\_write}, \texttt{i2c\_read}, \texttt{i2c\_stop}), no una capa intermedia de ``registro + datos''.
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Contenido} (N bytes): en respuestas de detección, cinco enteros de 16 bits (véase sección 4.4).
  \item \textbf{Checksum} (1 byte): suma de todos los bytes anteriores del frame, módulo 256.
\end{itemize}
El ``tipo'' de detección (línea, tag, color, etc.) se infiere del comando (bloque vs.\ flecha) y del algoritmo seleccionado en el HuskyLens. La codificación y decodificación está en \texttt{HuskyLensProtocolCore}, compartida conceptualmente con la librería Arduino.
...
El mecanismo implementado replica conceptualmente el comportamiento observado en la librería oficial de Arduino para HuskyLens, adaptándolo al entorno ESP32/FreeRTOS y al modelo de acceso I2C utilizado por Lua-RTOS.
...
(\textit{leftovers}) entre lecturas consecutivas.
  \item \textbf{Capa de acceso en C} (\texttt{huskylens.c}): se adjunta al bus I2C, realiza el handshake, envía peticiones y procesa las respuestas; delega la codificación de frames en el núcleo de protocolo y utiliza funciones auxiliares para la gestión del bus y la temporización (véanse apartados 4.2 y 4.3.4).
...
Todas las funciones de petición (solicitar todos los resultados, solo bloques,
solo flechas, por identificador, objetos aprendidos, etc.) siguen el mismo
...
El detalle de cada función (firmas y parámetros) se encuentra en el código fuente del proyecto.


\section{Uso desde la aplicación (C y Lua)}

\textbf{Desde C.} La aplicación declara una estructura de estado (\texttt{huskylens\_t}), llama a \texttt{huskylens\_init()} y luego utiliza las funciones de petición, acceso y configuración descritas en la sección 4.6.

\textbf{Desde Lua-RTOS.} El módulo se carga como librería (p.\,ej. \texttt{huskylens = require("huskylens")}) y se usa la misma instancia global. Uso típico:
\begin{enumerate}[noitemsep]
...
El propósito fundamental de este módulo es cumplir con el requerimiento funcional de transferencia de configuración (RF-05), permitiendo que el docente envíe secuencias de etiquetas válidas de forma inalámbrica desde la aplicación web. Esta conexión dinámica transforma al robot de un autómata con rutas fijas en una plataforma adaptable a diversos desafíos pedagógicos.
...
Un BT se ejecuta mediante \emph{ticks} periódicos. En cada tick se evalúan sus nodos desde la raíz, y cada nodo decide si:
...
Si bien algunos motores BT incorporan nodos \emph{Parallel}, estos no se utilizan en este proyecto debido al carácter secuencial y determinista del entorno embebido sobre ESP32.
...
\begin{itemize}
    ...
    \begin{figure}[h]
        ...
        \caption{Secuencia con memoria}
        \label{fig:mi_etiqueta}
    \end{figure}
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/SequenceCompositionWithMemoryEmulation.png}
        \caption{BT que emula la ejecucion de una secuencia con memoria usando nodos sin memoria}
        \label{fig:mi_etiqueta}
    \end{figure}
...
\begin{verbatim}
Intentar obtener comida del refrigerador
→ Si falla, ordenar comida
→ Si falla, fallback a "pasar hambre"
\end{verbatim}

El selector prueba opciones en orden de prioridad hasta que una resuelva la tarea.
...
\subsection{Implementación del Árbol de Comportamiento}
\label{subsec:implementacion_del_arbol_de_comportamiento}
...
\begin{itemize}
    \item \textbf{\texttt{sequence(children)}}: Avanza de izquierda a derecha y solo pasa al siguiente hijo si el anterior devuelve \texttt{SUCCESS}. Si un hijo devuelve \texttt{RUNNING}, la secuencia se detiene y propaga ese estado. Si un hijo devuelve \texttt{FAILURE}, el nodo reinicia su puntero interno \texttt{self.counter} y ejecuta la función reset en todos sus hijos para garantizar modularidad en el próximo ciclo. La implementacion de este nodo es con memoria. Una vez que un hijo tiene éxito, el nodo no vuelve a enviarle un "tick" en los ciclos siguientes. En su lugar, utiliza un puntero interno \texttt{self.counter} para saltar directamente al hijo que quedó en estado de ejecución o al siguiente en la lista
   
    \item \textbf{\texttt{selector(children)}}: Busca la primera alternativa exitosa. Ticks a sus hijos en orden; si uno devuelve \texttt{SUCCESS}, el selector termina con éxito inmediatamente. Si un hijo devuelve \texttt{RUNNING}, la secuencia se detiene y propaga ese estado. Devuelve \texttt{FAILURE} únicamente si todos sus hijos fallan. Al igual que la secuencia, limpia su memoria interna y resetea a los hijos al finalizar con éxito o tras agotar las opciones. La implementacion de este nodo es con memoria. Si los primeros hijos de un Selector fallan, el nodo recuerda esos fallos y no vuelve a intentarlos en los siguientes ticks, centrándose únicamente en la alternativa que devolvió RUNNING o en las restantes
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Reducción del costo computacional}: al mantener el puntero interno \texttt{self.counter} hacia el hijo que devolvió \texttt{RUNNING}, el sistema no malgasta recursos re-evaluando ramas del árbol que ya terminaron su labor. Esto permite que el árbol funcione de manera más fluida en sistemas con recursos limitado
   
    \item \textbf{Continuidad de acciones en estado RUNNING}: técnicamente, en la implementación del modulo, cuando un hijo devuelve \texttt{RUNNING}, el nodo padre debe devolver lo mismo inmediatamente para ceder el control. Por lo tanto, la memoria es vital aquí para que, en el siguiente tick, el árbol sepa exactamente dónde retomar el trabajo. Sin memoria, el ``tick'' volvería al primer hijo, lo que podría interrumpir la acción que estaba en curso o causar comportamientos erráticos
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Si existen errores, ilumina un patrón combinado que refleja el estado de cada estación evaluada. Se divide el anillo LED en la misma cantidad de segmentos que estaciones visitadas. Luego, las estaciones que se encuentren en orden correcto mostrarán un color \textbf{verde} y las estaciones que no se encuentren en el orden incorrecto mostrarán un color \textbf{roja}.
    \end{itemize}
...
\subsection{Implementacion del juego utilizando BT}
...
% dimensiones, componentes a alojar, campo de visión de la HuskyLens, fabricación
...
\subsection{Fabricación y ensamblado}
% material, impresión 3D u otra técnica, montaje en el robot

// Completar luego de realizado

\subsection{Validación del prototipo}
% ajuste mecánico, acceso a componentes, pruebas de funcionamiento
...
El resultado obtenido se detalla a continuación, donde se puede observar el modelo terminado con ambas piezas (carcasa y tapa) ya ubicados en sus respectivos lugares \\
...
Como trabajo a futuro queda realizar un rediseño del mismo para mejorar su aspecto estético, y realizar pruebas estructurales para verificar su durabilidad.
...
\subsubsection{Sistema de tarjetas con códigos QR}
...
\begin{itemize}
    \item Cambio de asociación de QR a pasos de la secuencia.
    \item App para identificar el orden de los QRs.
    \item Feedback con sonido.
    \item Bifurcaciones en el mapa.
\end{itemize}
